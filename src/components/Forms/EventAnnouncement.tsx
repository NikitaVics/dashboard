  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement> | ChangeEvent<HTMLSelectElement>) => {
    ...
    const handleSendClick = async () => {
      try {
        ...
        const response = await ky.post(`${process.env.NEXT_PUBLIC_API_BASE_URL}Management/Announcement/create EventAnnouncement`, {
          body: data,
        });
    ...
    const handleSchedule = async () => {
      try {
        ...
        const scheduledDateTime = new Date(`${selectedScheduledDate}T${selectedScheduledTime}:00.000Z`).toISOString();
    ...
    return (
        ...
        borderRadius={"20px"}>
            <GridItem rowSpan={1} colSpan={2}>
              ...
              <Input  type="text" value={eventName}  bgColor = {bgColor}  placeholder="Event Name"
                               
                               _placeholder={{ color: "rgba(124, 124, 125, 1)" }} onChange={handleMessageChange}/>
            </GridItem>
...
>
  ...
  >
    ...
    />
   <Flex>
    {eventData?.images && typeof eventData.images === 'string' ? (
      <Image src={eventData.images} alt="Attached Image" ml={1} w="150px" />
    ) : (
