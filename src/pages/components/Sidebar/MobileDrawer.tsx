...
function MobileDrawer() {
  ...
  const Title = ({
    ...
    <Flex px="5" py="3" alignItems="center">
      {leftIcon}
      {
        <Text fontSize="sm" fontWeight="400" borderRadius="4px">
          {text}
        </Text>
      }
    </Flex>
  ...
  return (
    <Box aria-label="leftNavigation" as="nav" color={"rgba(124, 124, 125, 1)"}>
            ...
            <DrawerBody>
              ...
              >
                ...
                {dashboards.map(({ path, icon, name }) => {
                  return (
                    <MenuItem key={path} name={name} icon={icon} path={path} />
                  );
                })}
              </List>
            </DrawerBody>
