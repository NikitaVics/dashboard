...
const bookings = () => {
  ...
  const { data: bookingDetails } = useSWR("/api/bookings/bookingInform")
  console.log("bookingDetails")
  
  
  
  const { data: cancelbookings } = useSWR("/api/bookings/cancelbooking")
...
<Box px={5}>
              ...
              <Box mt={5}>
                  <Table columns={columnConfig} data={bookingDetails} />   
              </Box>
              {/* {isEditModalOpen && (
              <><MembersForm
                    isOpen={isEditModalOpen}
                    onClose={() => setIsEditModalOpen(false)} memberId={memberId} memberId={memberId} /></>
              )}  */}
